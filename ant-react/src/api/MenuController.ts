...
export interface MenuInfoByIdVO {
    authFlag?: 'true' | 'false' // 是否是权限菜单，权限菜单：不显示，只代表菜单权限
    ...
    firstFlag?: 'true' | 'false' // 是否是起始页面，备注：只能存在一个 firstFlag === true 的菜单
    icon?: string // 图标
    id?: number // 主键id
    linkFlag?: 'true' | 'false' // 是否外链，即，打开页面会在一个新的窗口打开，可以配合 router
    name?: string // 菜单名
    ...
    router?: string // 路由
    showFlag?: 'true' | 'false' // 是否显示在 左侧的菜单栏里面，如果为 false，也可以通过 $router.push()访问到
